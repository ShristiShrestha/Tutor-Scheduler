    ...
    <NeedsTutoring>
        ...
        <StatusTagList>
            ...
            {subjects && subjects.length < 1 &&
                <ResText14Regular className={"text-grey2"}>
                    <i>
                        {title === "Specializations" ? "- No specific specialization mentioned." : "- No specific course mentioned for tutoring."}
                    </i>
                </ResText14Regular>}
        </StatusTagList>
        {studentNote && (
    ...
    const getCanRate = () => {
        if (appointment) {
            ...
            if (appointment.status === AppointmentStatus.PENDING)
                return [false, "The appointment is still in pending. Please wait til the tutor accepts your request."]
        ...
        if (menuItems.length > 1)
            switch (defaultTab) {
                case menuItems[1].key:
                    return (
                        <TabContent>
                            <ResText14Regular className={!rateResponse[0] ? "text-italic text-grey2" : ""}>
                                {appointment.rating > 0 ?
                                    <CheckCircleFilled className={"rated-icon rated-icon-done"}
                                    /> : !rateResponse[0] ?
                                        <InfoCircleOutlined className={"rated-icon rated-icon-not-allowed"}
                                        /> : ""}
                                {rateResponse[1]}
                            </ResText14Regular>
                            <div className={"rate-tutor-content"}>
                                ...
                                >
                                    ...
                                    <ul className={"rate-tutor-options"}>
                                        {ratingOptions.map((item, index) => {
                                            const eitherSelectedOrAlreadyRated = (appointment.rating === index + 1) || (rateResponse[0] && (index + 1 === rateRequest.rating));
                                            return <Tooltip title={item.title}>
                                                <li
                                                    ...
                                                    onClick={() =>
                                                        rateResponse[0] && handleRateChanges(
                                                            "rating",
                                                            index + 1,
                                                        )
                                                    }
                                ...
                                {rateResponse[0] && <div className={"h-start-flex"}>
                                    <MyButton
                                        type={"primary"}
